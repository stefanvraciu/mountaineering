...
<html lang="en">
	          ...
	          <span class="navbar-toggler-icon"></span>
                ...
                ?>
	            	...
	            	<li class="nav-item active">
	              		<a class="nav-link" href="#">Mountains
    ...
    <header class="business-header img-2">
      <div class="container">
        <div class="row">
          <div class="col-lg-12">
            <h1 class="display-3 text-center text-white mt-4"><strong>Search mountains of the world</strong></h1>
          </div>
        </div>
      </div>
    </header>
     ...
     <!--Search form-->
     <div class="container">
        <p>Here you can search the database for mountains. You can search by name, height or country using the form below.</p><br>
        ...
        <form id="inForm">
          ...
          <div class="form-check form-check-inline">
            <input class="form-check-input" type="radio" name="criteria" id="heightdown" value="heightdown">
            <label class="form-check-label" for="heightdown">
              By height (below)
            </label>
          </div>
          <div class="form-check form-check-inline">
            <input class="form-check-input" type="radio" name="criteria" id="heightup" value="heightup">
            <label class="form-check-label" for="heightup">
              By height (above)
            </label>
          </div>
          <div class="form-check form-check-inline">
...
$(document).ready(function(){ 
    ...
    {  
        ...
        $.ajax({  
            ...
            method:"POST", 
            data:{table:'mountains', criteria:criteria, qdata:querydata},
            dataType: "text",   
    ...
    $(document).on('click', '#btn_add', function(){  
        var mtname = $('#mtnname').text();  
        var mthgt = $('#mtnhgt').text();
        var mtloc = $('#mtnloc').text();    
        if(mtname == '')  
        {  
        ...
        if(mthgt == '')  
        {  
            alert("Enter Mountain height");  
            return false;  
        } 
        if(mtloc == '')  
        {  
            alert("Enter Mountain location");  
            return false;  
        } 
        $.ajax({  
            url:"../scripts/insert.php",  
            method:"POST",  
            data:{mtname:mtname, mthgt:mthgt, mtloc:mtloc, table:'mountains'},  
            dataType:"text",  
            success:function(data)  
            {  
                alert(data);  
  ...
  function edit_data(id, text, column_name)  
    {  
        $.ajax({  
            ...
            method:"POST",  
            data:{id:id, text:text, column_name:column_name, table:'mountains'},  
            dataType:"text",  
            success:function(data){  
    ...
    $(document).on('blur','.mtnname', function(){  
        var id = $(this).data("id1");  
        var mname = $(this).text(); 
        edit_data(id, mname, "Name");
    });  
    $(document).on('blur', '.mtnhgt', function(){  
        var id = $(this).data("id2");  
        var mhgt = $(this).text();  
        edit_data(id,mhgt, "Height");  
    });
    $(document).on('blur', '.mtnloc', function(){  
        var id = $(this).data("id3");  
        var mloc = $(this).text();  
        edit_data(id,mloc, "Location");  
    });    
